...
class Application(tkinter.Frame):
    ...
    def createWidgets(self):
        ...
        # ctrlSerialButton
        self.ctrlSerialButton = tkinter.Button(self.frameLeft, text='Open', bg='Green', command=self.ctrlSerialButtonFunction)
        ...
        self.statusValueLabel.grid(row=0, column=0,sticky="wesn")
    ...
    def ctrlSerialButtonFunction(self):
    ...
    def updateSerial(self):
        ...
        while self.serialStatus == True:
            self.serialData = self.serial.readline()
            self.serial.write(self.serialData)
            self.statusValue.set('[%s]' %(self.serialData))
            
            # n = self.serial.inWaiting()
            # if n >= 11:
                # self.serialData = self.serial.read(1)
                # if self.serialData[0] != 0xa7:
                    # continue
                # self.serialData += self.serial.read(1)
                # if self.serialData[1] != 0x7a:
                    # continue
                
                # # Datas
                # self.serialData += self.serial.read(9)
                # # self.serial.write(self.serialData)
                
                # self.angleValueX = struct.unpack('<h',self.serialData[5:7:1])
                # self.angleValueY = struct.unpack('<h',self.serialData[7:9:1])
                # self.angleValueZ = struct.unpack('<h',self.serialData[9:11:1])
                # # Display
                # xdata = bytes(binascii.b2a_hex(self.serialData))
                # self.receiveValue.set(xdata.decode('ascii').upper())
...
###########################################################
                ...
                # vbyte=binascii.b2a_hex(self.serialData[8:6:-1])
                # value=int(bytes(vbyte).decode('ascii'), 16)
                # self.angleValueY=ctypes.c_int16(value).value
                
                # vbyte=binascii.b2a_hex(self.serialData[10:8:-1])
                # value=int(bytes(vbyte).decode('ascii'), 16)
