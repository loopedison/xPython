...
class Application(tkinter.Frame):
    ...
    def createWidgets(self):
        ...
        # decryptButton
        self.decryptButton = tkinter.Button(self.frameLeft, text='decrypt', command=self.decryptButtonCallBack)
        self.decryptButton.grid(row=0, column=4, rowspan=2, columnspan=1, sticky="wesn")
    ...
    def encryptButtonCallBack(self):
        ...
        pass
    def decryptButtonCallBack(self):
        if self.inFilePath != None and self.inFilePath != '' and self.outFilePath != None and self.outFilePath != '':
            xxtea.decrypt(self.inFilePath, self.outFilePath)
            self.statusValue.set('[%s]' %('decrypt inFile => outFile Success!'))
        else:
            self.statusValue.set('[%s]' %('File Failed!'))
        pass
#
